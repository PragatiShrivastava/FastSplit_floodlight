...
void *runner(void *params){
	...
	while(1){
		...
		// if(len = read(s_a, buf, sizeof(buf)) && printf("%d\n", len)  && len > 0){
                printf("Value of ssize_t : %d\n", recvfrom(s_a, buf, 65536,0,&saddr,(socklen_t *)&saddr_len));
...
int main (void)
{
	...
	strncpy(ifreq_obj_a.ifr_name, "h3-eth1",
			sizeof(ifreq_obj_a.ifr_name) - 1);/* change from "mal_host_port"*/
	...
	strncpy(ifreq_obj_b.ifr_name, "h3-eth0",   
			sizeof(ifreq_obj_b.ifr_name) - 1);  /* change from "mal_host_out"*/
